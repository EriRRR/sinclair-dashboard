...
const Fincas = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (editando) {
      const { error } = await supabase.from('fincas').update(formData).eq('id', editando)
      if (error) toast.error(error.message)
      else toast.success('Finca actualizada')
    } else {
      const { error } = await supabase.from('fincas').insert([formData])
      if (error) toast.error(error.message)
      else toast.success('Finca creada')
    }
    setModalOpen(false)
    setEditando(null)
    setFormData({ nombre: '' })
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-white p-6 rounded-lg w-96">
            <h2 className="text-xl font-bold mb-4">{editando ? 'Editar' : 'Nueva'} Finca</h2>
