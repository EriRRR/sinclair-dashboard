...
const Labores = () => {
  ...
  const cargar = async () => {
    const { data, error } = await supabase.from('labores').select('*').order('nombre')
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (editando) {
      const { error } = await supabase.from('labores').update(formData).eq('id', editando)
      if (error) toast.error(error.message)
      else toast.success('Labor actualizada')
    } else {
      const { error } = await supabase.from('labores').insert([formData])
      if (error) toast.error(error.message)
      else toast.success('Labor creada')
    }
    setModalOpen(false)
    setEditando(null)
    setFormData({ nombre: '' })
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-white p-6 rounded-lg w-96">
            <h2 className="text-xl font-bold mb-4">{editando ? 'Editar' : 'Nueva'} Labor</h2>
