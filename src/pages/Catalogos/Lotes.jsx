...
const Lotes = () => {
  ...
  const [formData, setFormData] = useState({ numero: '', finca_id: '' })
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (editando) {
      const { error } = await supabase.from('lotes').update(formData).eq('id', editando)
      if (error) toast.error(error.message)
      else toast.success('Lote actualizado')
    } else {
      const { error } = await supabase.from('lotes').insert([formData])
      if (error) toast.error(error.message)
      else toast.success('Lote creado')
    }
    setModalOpen(false)
    setEditando(null)
    setFormData({ numero: '', finca_id: '' })
    cargarLotes()
  }
            ...
            {items.map(item => (
              <tr key={item.id}>
                ...
                <td className="px-6 py-4 flex gap-2">
                  <button onClick={() => { setEditando(item.id); setFormData({ numero: item.numero, finca_id: item.finca_id }); setModalOpen(true) }}><Edit size={18} className="text-blue-600" /></button>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-white p-6 rounded-lg w-96">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <input type="text" placeholder="Número de lote" className="w-full border p-2 rounded" value={formData.numero} onChange={e => setFormData({ ...formData, numero: e.target.value })} required />
              <select className="w-full border p-2 rounded" value={formData.finca_id} onChange={e => setFormData({ ...formData, finca_id: e.target.value })} required>
